...
export function GlobalSearch({ compact = false }: GlobalSearchProps) {
  ...
  function openMarketplaceSearch() {
    const trimmed = query.trim();
    setOpen(false);
    router.push(buildMarketplaceHref({ keyword: trimmed, city: "", district: "", categoryId: "" }));
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-1 top-1/2 flex -translate-y-1/2 items-center gap-0.5">
        ...
        <button
          type="button"
          className="rounded-lg p-2 text-violet-200/90 transition-colors hover:bg-violet-500/20"
          aria-label="Ara"
      ...
      {showDropdown ? (
        <div
          className={`${orivonaDropdownScroll} absolute left-0 right-0 top-[calc(100%+0.5rem)] z-[9600] max-h-[min(24rem,70vh)] overflow-y-auto overflow-x-hidden rounded-2xl border border-violet-200/15 bg-[#0c0814]/98 p-2 shadow-[0_24px_64px_-12px_rgba(0,0,0,0.85)] backdrop-blur-xl`}
          role="listbox"
        >
          {hits.length === 0 && !loading ? (
            <div className="space-y-2 px-2 py-3">
              ...
              <button
                type="button"
                className="w-full rounded-xl border border-violet-400/25 bg-violet-500/10 px-3 py-2 text-sm text-violet-100 hover:bg-violet-500/20"
                onClick={openMarketplaceSearch}
              >
                Marketplace&apos;te ara: &quot;{query.trim()}&quot;
              </button>
            </div>
          ) : (
            <>
              ...
              <button
                type="button"
                className="mt-1 w-full rounded-xl border border-white/10 px-3 py-2 text-xs text-violet-200/90 hover:bg-white/[0.04]"
                onClick={openMarketplaceSearch}
              >
